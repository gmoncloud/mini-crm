  ...
  <div class="p-6 flex space-x-2">
    ...
    <div class="flex-1">
      <div class="flex justify-between items-center">
        <div>
          <span class="text-gray-800">{{ property_type.name }}</span>
          <small class="ml-2 text-sm text-gray-600">{{ dayjs(property_type.created_at).fromNow() }}</small>
          <small v-if="property_type.created_at !== property_type.updated_at" class="text-sm text-gray-600"> &middot; edited</small>
        </div>
<!--        <Dropdown v-if="chirp.user.id === $page.props.auth.user.id">-->
      ...
      <form v-if="editing" @submit.prevent="form.put(route('property_types.update', property_type.id), { onSuccess: () => editing = false })">
      ...
      <p v-else class="mt-4 text-lg text-gray-900">{{ property_type.name }}</p>
    </div>
  </div>
</template>